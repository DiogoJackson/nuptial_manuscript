...
claw <- reflet_claw %>%
  ...
  geom_ribbon(aes(ymin = media3 - sd3, ymax = media3 + sd3, fill = "Darkened white"), alpha = 0.3, show.legend = FALSE) +
  geom_line(aes(y = media3, color = "Darkened white"), size = 1) +
  
  scale_fill_manual(values = c("White" = "#ffb560", 
                               "Dark" = "black",
                               "Darkened white" = "grey30"), name = "") +
